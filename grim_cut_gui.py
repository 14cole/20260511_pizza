...
class GrimCutWindow(DatasetOpsMixin, PlotOpsMixin, QMainWindow):
    def __init__(self) -> None:
        ...
        for tab_key, context in self._plot_contexts.items():
            ...
            context.chk_isar_peak_normalize.toggled.connect(self._on_isar_window_changed)
    ...
    def _build_plot_left_context(self, panel: QWidget) -> PlotContext:
        ...
        combo_isar_algorithm.addItems(["Decoupled FFT", "Polar Format", "Back-Projection"])
        ...
        chk_isar_peak_normalize = QCheckBox("Peak-normalize")
        chk_isar_peak_normalize.setToolTip(
            "If checked: brightest pixel in the image is set to 0 dB and "
            "everything else displays relative to it. Otherwise the values "
            "are absolute dBsm (a unit-amplitude scatterer reads 0 dB)."
        )
        settings_layout.addWidget(chk_isar_peak_normalize, row, 4, 1, 2)
        row += 1
        ...
        return PlotContext(
            ...
            chk_isar_peak_normalize=chk_isar_peak_normalize,
